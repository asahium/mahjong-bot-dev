#!/usr/bin/env python3
"""
DQN Training Script for Riichi Mahjong

Train a DQN agent to play Riichi Mahjong (EMA or Tenhou rules).
...
from agents.sb3_agent import SB3Agent, RiichiFeaturesExtractor
...
def train_riichi_dqn(
    ...
    opponent_policy: str = "random",
    reward_shaping: bool = True,
    save_dir: str = "models/riichi_dqn",
    eval_freq: int = 10000,
    seed: int = 42,
    device: str = "auto",
    verbose: int = 1,
    ...
    """Train a DQN agent for Riichi Mahjong."""
    
    # Create save directory
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    run_name = wandb_run_name or f"dqn_{rules}_{timestamp}"
    ...
    print("=" * 60)
    print(f"Riichi Mahjong DQN Training ({rules.upper()} rules)")
    print("=" * 60)
    ...
    checkpoint_callback = CheckpointCallback(
        ...
        name_prefix=f"riichi_dqn_{rules}",
    )
...
def main():
    parser = argparse.ArgumentParser(description="Train DQN for Riichi Mahjong")
    ...
    # Saving parameters
    parser.add_argument("--save-dir", type=str, default="models/riichi_dqn")
